...
public class RegisterWindowZak {
	...
	public void actionEvents() {
		...
		if (ZakLauncher.getUserController().userExists(userField.getText())) {
		...
		//If a user name does not already exist
			...
			if (!(userField.getText().isEmpty()) && !(passField.getText().isEmpty())) {
				UserZak user = new UserZak (userField.getText(), passField.getText());
				ZakLauncher.getUserController().addUser(user);
				StateUtil.saveInformation(ZakLauncher.getUserController());
				...
				msg.showAndWait();
				System.out.println(ZakLauncher.getUserController());
				new LoginWindowZak();
